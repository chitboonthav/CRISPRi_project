...
test_lrt = function_edgeR_GLM(counttable = count_BW, cutoffCPM = 6)
res_lrt_BW = test_lrt$table

test_lrt = function_edgeR_GLM(counttable = count_BL, cutoffCPM = 6)
...
write.csv(x = res_lrt_BW_merge, file = "res_lrt_BW_merge_CPM6.csv",row.names = FALSE)
write.csv(x = res_lrt_BL_merge, file = "res_lrt_BL_merge_CPM6.csv",row.names = FALSE)
...
BW_indi_sgRNA = read.csv(file = "res_lrt_BW_merge_CPM6.csv", header = TRUE)
BL_indi_sgRNA = read.csv(file = "res_lrt_BL_merge_CPM6.csv", header = TRUE)
...
p2
pdf(file = "scattteredplot_randsgRNA_BW_CPM6.pdf",width = 8, height = 6)
p2
dev.off()
...
p2 = ggplot(data = BL_rand , aes(x = sgRNA, y = logFC_BL)) +
  ...
  theme(strip.background=element_blank())
p2
pdf(file = "scatteredplot_randsgRNA_BL_CPM6.pdf",width = 8, height = 6)
p2
dev.off()
...
p1 = ggplot(data = BW_rand, aes(x = logFC_BW)) +
  ...
  theme(strip.background=element_blank())
p1
pdf(file = "20240103_dist_randomized_BW_CPM6_unpaired_logFC.pdf",width = 8, height = 6)
p1
dev.off()
...
pdf(file = "20240103_BW_CPM6_filterRandom_v2.pdf", width = 9, height = 6.5)
print(testrun_BW)
dev.off()
...
pdf(file = "20230103_BL_CPM6_filterRandom_attheEnd_v2.pdf", width = 9, height = 6.5)
print(testrun_BL)
dev.off()
